...
const getGradientStop = (ratio: number) => {
  ...
  const leftParts = LEFT_COLOR.match(/.{1,2}/g) ?? ["00", "00", "00"];
  const rightParts = RIGHT_COLOR.match(/.{1,2}/g) ?? ["00", "00", "00"];
  const c0 = leftParts.map((oct) => parseInt(oct, 16) * (1 - ratio));
  const c1 = rightParts.map((oct) => parseInt(oct, 16) * ratio);
